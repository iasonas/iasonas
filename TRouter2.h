//Iasonas
#ifndef __TROUTER2_H__
#define __TROUTER2_H__
...
SC_MODULE(TRouter2)
{
  ...
  // I/O Ports
  //*****No ports for local connections-No PE

  sc_in_clk          clock;        // The input clock for the router
  sc_in<bool>        reset;        // The reset signal for the router

  sc_in<TFlit>       flit_rx[DIRECTIONS_2];   // The input channels (including local one)
  ...
  sc_out<TFlit>      flit_tx[DIRECTIONS_2];   // The output channels (including local one)
  ...
  // Neighbor-on-Path related I/O Eixe Directions oxi Directions+1 opos pano
  ...
  */
  ...
  int                selection_type;
  TBuffer            buffer[DIRECTIONS_2];            // Buffer for each input channel 
  bool               current_level_rx[DIRECTIONS_2];  // Current level for Alternating Bit Protocol (ABP)
  bool               current_level_tx[DIRECTIONS_2];  // Current level for Alternating Bit Protocol (ABP)
